...
def receive(tb, samp_rate, freq):
    ...
    tb.osmo_source.set_center_freq(freq)
    ...
    print(hasattr(tb, 'osmo_source'), 'osmo source')
    
    
def transmit(tb, samp_rate, freq, song):
    ...
    tb.wavfile_source = wavfile_source(song, True)
    ...
    tb.osmo_sink.set_center_freq(freq)
    ...
    if hasattr(tb, 'tracker'):
        tb.unlock()
    print(hasattr(tb, 'tracker'), 'tracker')
    
    

def dostuff(tb, samp_rate, freq, song):

    samp_rate = int(samp_rate)
    window = tk.Tk()
    window.bind("<t>", lambda x: transmit(tb, samp_rate, freq, song))
    window.bind('<r>', lambda x: receive(tb, samp_rate, freq))
    label = tk.Label(window, text="Press 't' to transmit, press r to receive")
    label.pack()
    window.mainloop()
    # receive(tb, samp_rate, freq)
    
    # transmit(tb, samp_rate, freq, song)

    input('Press Enter to quit')


def main():
    song = '/home/usacys/Desktop/Paul/imperial_march.wav'
    tb = gr.top_block()
